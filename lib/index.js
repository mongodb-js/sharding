...
var debug = require('debug')('mongodb-sharding');
  ...
  db.db('config').collection('shards')
    ...
    .toArray(function(err, docs) {
      if (err) {
        if (isNotAuthorized(err)) {
          debug('not authorized to get metadata to '
            + 'discover replicaset instances. omitting.');

          var serverUrl = _.has(db, 'serverConfig.s.options.url') ?
            db.serverConfig.s.options.url : false;
          if (serverUrl) {
            return fn(null, [{ _id: Instance.getId(serverUrl) }]);
          }
          return fn(null, []);
        }
        return fn(err);
      ...
      var res = _.chain(docs)
        .pluck('host')
        .map(function(url) {
          if (!url) {
            return null;
          }
          var slashes = url.split('/')[1];
          if (!slashes) {
            return null;
          }

          return slashes.split(',').map(function(d) {
            return {
              _id: Instance.getId(d)
            };
          });
        })
        .filter(function(d) {
...
exports.getBalancer = function(db, fn) {
  ...
  db.db('config')
    ...
    .findOne(query, function(err, doc) {
      ...
      db.db('config')
        .collection('locks')
        .findOne(query, function(_err, lock) {
          if (_err) {
            return fn(_err);
          }
          if (doc) {
            running = lock.state > 0;
          }
          fn(null, {
            state: state,
            running: running
